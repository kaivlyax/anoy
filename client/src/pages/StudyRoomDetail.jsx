...
export default function StudyRoomDetail() {
  ...
  const fetchRoomData = useCallback(async () => {
    try {
      setLoading(true);
      const res = await studyRoomApi.getRoom(roomId);
      if (res.data.success) {
        setRoom(res.data.room);
      }
    } catch (err) {
      console.error("Fetch room error:", err);
      addToast(err.response?.data?.message || "Study room not found", "error");
  ...
  useEffect(() => {
    ...
    socket.emit(
      ...
      (res) => {
        if (!res?.success) {
          addToast(res?.message || "Failed to join study room", "error");
          navigate("/study-rooms");
          return;
        }
  ...
  }, [socket, isConnected, roomId, location.state, navigate, addToast, handleRemoteStream, handlePeerLeft]);
  ...
  const handleToggleMic = () => {
    ...
    if (socket && isConnected) {
      socket.emit("study_room:state_change", {
        roomId,
        isMuted: nextState
      });
    }
    ...
    <div className="study-room-conference-view">
      ...
      <header className="conference-header">
        <div className="conference-info">
          <Link to="/study-rooms" className="conference-back-btn" title="Back to Lobby">
            ←
          </Link>
          <div>
            <div className="conference-title-row">
              <GraduationCapIcon size={20} className="study-header-icon" />
              <h2 className="conference-title">{room.title}</h2>
              <span className="conference-topic-pill">{room.topic}</span>
            </div>
            <span className="conference-subtext">
              {room.description || "Active collaboration study room"}
            </span>
          </div>
        </div>
        ...
        <div className="conference-header-right">
          <div className="live-status-pill">
            ...
            <span>LIVE ({totalParticipantsCount})</span>
          </div>
        </div>
      </header>
